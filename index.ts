...
let personName3: string = "William Sons";
...
let titleCase = (string: string) => {
    ...
    let sentence = string.trim().toLowerCase().split(" ");
...
let message: string = `${famous_person} once said, " THE STRONG PERSON IS NOT THE ONE WHO CAN WRESTLE SOMEONE ELSE DOWN THE STONG PERSON IS THE ONE WHO CAN CONTROL HIMSELF WHEN HE IS ANGRY."`;
...
let personName2: string = "\t Ali Afaq \n";
...
let names: string[] = ["Ali", "Mirza", "Hammad", "Ahmad"];
...
let car: string[] = ["Rolls Royce", "lamborghini", "ferrari", "Honda"];
for (let i: number = 0; i < car.length; i++) {
    //now print the friends name with the message
    console.log(`I would like to own a  ${car[i]} car In Sha Allah one day.`);
}

console.log("Question No 14");
//Guest List: If you could invite anyone, living or deceased, to dinner, who would you invite? Make a list that includes at least three people you’d like to invite to dinner. Then use your list to print a message to each person, inviting them to dinner.

let list: string[] = ["Sharjeel", "Farhan", "Ali", "Hammad"];
for (let i: number = 0; i < list.length; i++) {
    console.log(`I would like to invite you, Mr. ${list[i]}, to dinner. Please come!`);
}
...
for (let i: number = 0; i < list.length; i++) {
    console.log(`You are the final guests which is to going  Mr. ${list[i]}, to dinner. Please come!`);
}
...
for (let i = 0; i < list.length; i++) {
    console.log(`"I inform you that,${list[i]} three new guest join us our dinner"`);
}
...
type PersonInfo = { name: string, age: number, isMarried: boolean }
...
let citiesArr: string[] = ["Lahore", "Sialkot", "Karachi", "Islamabad"];
for (let i: number = 0; i <= citiesArr.length; i++) { //Removing the '=' from the condition in the index eliminates the index error, as the length function starts from 1 while the array begins at 0.
...
let driverName: string = "Ali";
let driverLicen: boolean = true;
let ownCar: boolean = false;
...
let A: string = "Alama Iqbal";
let B: string = "Alama Iqbal";
...
let sweets: string[] = ["biscuits", "chai", "cofee"];
...
let usernames: string[] = ["usama", "umar", "ahmad", "admin", "ibrahim"];
